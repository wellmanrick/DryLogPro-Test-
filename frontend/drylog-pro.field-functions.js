...
async function _dlpWizRenderStep(){
  ...
  const progWrap = el('div',{style:'height:5px;background:#e2e8f0;border-radius:3px;margin-bottom:12px;overflow:hidden;'});
  progWrap.appendChild(el('div',{style:`height:100%;background:#3b82f6;width:${pct}%;border-radius:3px;transition:width 0.3s;`}));
  screen.appendChild(progWrap);
  screen.appendChild(el('div',{style:'font-size:11px;color:#64748b;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:4px;'}, `Step ${Math.min(st.idx+1, total)} of ${total}`));
  screen.appendChild(el('div',{class:'h1',style:'margin-bottom:14px;'}, step.label));

  root.appendChild(screen);

  if (step.type === 'outdoor_ref') {
    _dlpWizRenderOutdoor(screen);
  } else if (step.type === 'chamber_atm') {
    _dlpWizRenderChamberAtm(screen, step);
  } else if (step.type === 'chamber_moisture') {
    await _dlpWizRenderChamberMoisture(screen, step);
  } else if (step.type === 'dehu') {
    _dlpWizRenderDehu(screen, step);
  } else if (step.type === 'photos') {
    await _dlpWizRenderPhotos(screen);
  } else if (step.type === 'done') {
    await _dlpWizRenderDone(screen);
  }
}
...
async function _renderDlpCadEditor(ctx){
  ...
  __dlpCad = {
    ...
    saveTimer: null,
    saveStatus: 'saved',
    svgRoot: null, contentG: null, statusEl: null,
    snapToGrid: true,
    ortho: true,
    showDims: true,
  ...
  const toolbar = el('div',{style:'display:flex;align-items:center;gap:4px;padding:8px 10px;background:#0f172a;color:#fff;overflow-x:auto;flex-wrap:nowrap;-webkit-overflow-scrolling:touch;border-bottom:1px solid #1e293b;'});
  ...
  // ── SVG canvas ───────────────────────────────────────────────────────────
  const canvasWrap = el('div',{style:'position:relative;flex:1;background:#fafafa;touch-action:none;overflow:hidden;height:calc(100vh - 230px);min-height:400px;'});
  const svg = sv('svg', {
  ...
  if (tool === 'door' || tool === 'window' || tool === 'opening'){
    ...
    if (wHit){
      ...
      if (tool === 'door'){
        ...
        __dlpCad.state.openings.push({wallIdx: wHit.wallIdx, position: wHit.t, width: 60});
      }
      _dlpCadMarkDirty();
      _dlpCadRender();
    }
...
function _dlpCadRender(){
  ...
  for (let i = 0; i < (st.openings||[]).length; i++){
    ...
    g.appendChild(sv('line',{
      ...
      stroke: isSel ? '#dc2626' : '#16a34a', 'stroke-width': 2, 'stroke-dasharray':'4 3',
    }));
  }
