<?php
// Inside-the-zone atmosphere readings.
...
if ($method === 'GET' && $id && $sub === 'history') {
    json_list(tc_drylog_edit_history($db, $cid, 'zone_atmosphere_readings', (int)$id));
}

if ($method === 'GET' && $id) {
    $s = $db->prepare("SELECT * FROM zone_atmosphere_readings WHERE id = ? AND company_id = ?");
    $s->execute([$id, $cid]);
    $row = $s->fetch();
...
if ($method === 'GET') {
    ...
    $s = $db->prepare("SELECT * FROM zone_atmosphere_readings WHERE " . implode(' AND ', $where) . " ORDER BY reading_at DESC, id DESC");
    $s->execute($params);
...
if ($method === 'POST') {
    ...
    $db->prepare("
        INSERT INTO zone_atmosphere_readings
            (company_id, claim_id, drying_zone_id, visit_id, reading_at,
             temp_f, rh_pct, gpp, dew_point_f, vapor_pressure_kpa,
             captured_by_user_id, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ")->execute([
        $cid, $claim_id, $zone_id, $visit_id, $reading_at,
        $temp_f, $rh_pct, $p['gpp'], $p['dew_point_f'], $p['vapor_pressure_kpa'],
        (int)$user['id'],
        isset($b['notes']) ? trim((string)$b['notes']) : null,
    ]);

    $new_id = (int)$db->lastInsertId();
    $alerts = tc_alerts_evaluate($db, $cid, $claim_id, 'zone_atmosphere_readings', $new_id);
    $s = $db->prepare("SELECT * FROM zone_atmosphere_readings WHERE id = ? AND company_id = ?");
